...
import { getSchedule, getTerms } from '@/features/teacher/api/teacherApi'
...
export function TeacherSchedulePage() {
  ...
  useEffect(() => {
    ...
    getTerms(payload.school_id, t).then((terms) => {
      if (terms.length > 0) setSelectedTermId(terms[0].id)
    })
  }, [])

  useEffect(() => {
    if (!teacherId || !token || !selectedTermId) return
    getSchedule(teacherId, selectedTermId, token).then(setSchedule).catch(console.error)
  }, [teacherId, token, selectedTermId])
  ...
  return (
    <div className="p-8 space-y-5">
      ...
      {schedule.length === 0 ? (
        <p className="text-sm text-muted-foreground">Ders programı bulunamadı.</p>
      ) : (
